...
class Kandinsky3Endpoint(BaseEndpoint):
    ROUTE_PREFIX = "/api/v1/kandinsky3"
    DEPLOYMENT_NAME = "kandinsky3"
    MIN_REPLICAS = 0
    MAX_REPLICAS = 3
    ...
    def load_model(self):
        """Load Kandinsky3 model"""
        print("Loading Kandinsky3 model...")
        
        # Find kandinsky model
        kandinsky_file = self.model_base_path / "kandinsky3-1.ckpt"
        
        if not kandinsky_file.exists():
            print(f"⚠ Kandinsky model not found at {kandinsky_file}")
            return {"model_type": "kandinsky3", "loaded": False}
        
        print(f"📦 Loading Kandinsky3 model: {kandinsky_file.name}")
        
        # Simulate loading (replace with actual model loading)
        # For actual implementation:
        # from diffusers import Kandinsky3Pipeline
        # self.pipeline = Kandinsky3Pipeline.from_single_file(str(kandinsky_file))
        # self.pipeline = self.pipeline.to("cuda")
        
        time.sleep(6)  # Kandinsky might take a bit longer
        
        return {
            "model_type": "kandinsky3",
            "model_file": str(kandinsky_file),
            "loaded": True
        }
    
    def predict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Run Kandinsky3 inference"""
    ...
    def save_metrics(self, request_data: Dict, response_data: Dict, 
        ...
        metrics = {
            ...
            "prompt": request_data.get("prompt", "")[:100],
    ...
    def create_app(self) -> FastAPI:
        ...
        async def generate(request: Request, background_tasks: BackgroundTasks):
            ...
            while not self.ready and waited < max_wait:
                ...
                if waited % 10 == 0:
                    print(f"⏳ Waiting for Kandinsky3 to load... ({int(waited)}s)")
