#!/usr/bin/env python3
"""
Deploy Object Detection endpoints (YOLO, etc.)
...
# Import the Object Detection app
try:
    from serve_object_detection_v2 import create_app
except ImportError as e:
    print(f"❌ Failed to import serve_object_detection_v2: {e}")
    print("   Make sure serve_object_detection_v2.py is in the same directory")
    sys.exit(1)


def deploy_object_detection(model_name: str, model_path: str, app_name: str = None):
    """Deploy an object detection model"""
    
    if app_name is None:
        app_name = f"object-detection-{model_name.replace('/', '-').replace('_', '-').replace('.', '-')}"
    
    print(f"🚀 Deploying Object Detection endpoint...")
    ...
    try:
        ...
        print(f"✅ Object Detection endpoint deployed successfully!")
        print(f"   Application: {app_name}")
        ...
        print(f"   # Detect objects in image:")
        print(f"   curl -X POST http://{head_ip}:8000/{app_name}/v1/detect \\")
        print(f"     -H 'Content-Type: application/json' \\")
        print(f"     -d '{{\"image\": \"<base64-encoded-image>\", \"confidence\": 0.25}}'")
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Object Detection model (YOLO, etc.)")
    parser.add_argument(
        "--model",
        type=str,
        default="yolov8n.pt",
        help="Model name or path (default: yolov8n.pt - will download from Ultralytics)"
    )
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    deploy_object_detection(model_name, model_path, args.app_name)
